...
const LibraryModal = ({ isOpen, onClose }: LibraryModalProps) => {
  const [selectedCategory, setSelectedCategory] =
    useState<string>("All readings");
  const [isAnimatingOut, setIsAnimatingOut] = useState(false);
  const [isAnimatingIn, setIsAnimatingIn] = useState(false);

  const categories = ["All readings", "Fiction", "Poetry", "Essays"];

  const filteredReadings = useMemo(() => {
    const filtered =
      selectedCategory === "All readings"
        ? READINGS
  ...
  return (
    ...
    >
      ...
      {/* Modal */}
      <div
        className={`relative bg-background rounded-[16px] md:rounded-[24px] w-full max-w-7xl h-[90dvh] max-h-[1000px] overflow-y-auto transition-all duration-300 p-3 md:p-5 ${
          isAnimatingOut
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center px-4 md:px-8 pt-8 md:pt-12 pb-5 md:pb-6 gap-4">
          ...
          <p className="text-lg md:text-xl text-muted-foreground max-w-2xl md:max-w-xl text-center leading-tight hidden md:block">
            Ray comes preloaded with a vault of 1000+ timeless readings for your
            journey, always available offline and constantly getting updated.
          </p>
        </div>
