...
const LABEL_STYLES: Record<string, LabelStyle> = {
  ...
  'ENLIGHTENMENT_FORMAL': {
    background: 'linear-gradient(135deg, rgba(211, 182, 137, 0.7) 0%, rgba(158, 132, 89, 0.5) 100%)',
  ...
  'VICTORIAN_POETRY': {
    background: 'linear-gradient(135deg, #B5A159 0%, #8E8867 50%, #66461F 100%)',
  ...
  'AMERICAN_REGIONAL': {
    background: 'linear-gradient(202.68deg, #59B578 6.84%, #5F80B0 57.02%, #924B4B 103.44%)',
...
const ReadingThumbnail: React.FC<ReadingThumbnailProps> = ({ 
  ...
  const style = LABEL_STYLES[label as keyof typeof LABEL_STYLES] || DEFAULT_STYLE;
  ...
  const renderDecorative = () => {
    ...
    switch (style.hasDecorative) {
        ...
        return (
          <div className="flex-1 flex items-start justify-start">
            <div
              ...
              style={{
                background: 'linear-gradient(203.13deg, #B5595B 2.62%, #5F80B0 54.9%, #D0D0D0 103.28%)'
              }}
            />
          </div>
        );
      ...
      case 'bottom-columns':
        return (
          <div className="flex-1 flex items-stretch justify-start gap-2">
            <div className="rounded-md w-full h-full" style={{ backgroundColor: 'rgba(255, 255, 255, 0.2)' }} />
            <div className="rounded-md w-full h-full" style={{ backgroundColor: 'rgba(255, 255, 255, 0.2)' }} />
            <div className="rounded-md w-full h-full" style={{ backgroundColor: 'rgba(255, 255, 255, 0.2)' }} />
          </div>
        );
    ...
    <div className={`relative w-full h-[170px] rounded-[12px] overflow-hidden ${className}`}>
      ...
      />
        ...
        {style.hasDecorative === 'bottom-gradient' && (
          <div 
            className="absolute left-0 right-0 bottom-0 rounded-b-[12px]"
            style={{ 
              height: '84px',
      ...
      {/* Text overlay */}
      <div className={`${getPositionClasses()} ${getTextAlignClasses()} gap-3`}>
        ...
        {title && author ? (
          <div className="flex flex-col">
            ...
            <p 
              className="text-sm leading-tight max-w-full break-words"
              style={{ 
                color: style.textColor,
                fontSize: '12px'
              }}
